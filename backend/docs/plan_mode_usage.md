...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in Allo（元枢）.
...
Allo（元枢） uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall Allo（元枢） prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with Allo（元枢）'s main prompt
...
- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom Allo（元枢）-style prompts**
...
- Custom prompts emphasize the same principles as Allo（元枢）'s main system prompt (clarity, action-oriented, critical rules)
