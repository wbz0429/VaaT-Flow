...
Allo（元枢） supports configurable MCP servers and skills to extend its capabilities, which are loaded from a dedicated `extensions_config.json` file in the project root directory.
...
For `http` and `sse` MCP servers, Allo（元枢） supports OAuth token acquisition and automatic token refresh.
...
MCP servers expose tools that are automatically discovered and integrated into Allo（元枢）'s agent system at runtime. Once enabled, these tools become available to agents without additional code changes.
