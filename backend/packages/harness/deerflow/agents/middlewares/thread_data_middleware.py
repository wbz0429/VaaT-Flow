...
class ThreadDataMiddleware(AgentMiddleware[ThreadDataMiddlewareState]):
    ...
    def _get_thread_paths(self, thread_id: str, user_id: str | None = None) -> dict[str, str]:
        ...
        Returns:
            Dictionary with workspace_path, uploads_path, outputs_path, and tmp_path.
        """
        if user_id:
            return {
                "workspace_path": str(self._paths.user_sandbox_work_dir(user_id, thread_id)),
                "uploads_path": str(self._paths.user_sandbox_uploads_dir(user_id, thread_id)),
                "outputs_path": str(self._paths.user_sandbox_outputs_dir(user_id, thread_id)),
                "tmp_path": str(self._paths.user_thread_tmp_dir(user_id, thread_id)),
                "user_root_path": str(self._paths.user_dir(user_id)),
                "user_skills_path": str(self._paths.user_skills_dir(user_id)),
            }
        ...
        return {
            ...
            "tmp_path": str(self._paths.sandbox_user_data_dir(thread_id) / "tmp"),
        }
    ...
    def _create_thread_directories(self, thread_id: str, user_id: str | None = None) -> dict[str, str]:
        ...
        """
        if user_id:
            self._paths.ensure_user_thread_dirs(user_id, thread_id)
        else:
            self._paths.ensure_thread_dirs(thread_id)
            tmp_dir = self._paths.sandbox_user_data_dir(thread_id) / "tmp"
