...
description: "Interact with the Allo（元枢） AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to Allo（元枢） for research/analysis, start an Allo（元枢） conversation thread, check Allo（元枢） status or health, list available models/skills/agents in Allo（元枢）, manage Allo（元枢） memory, upload files to Allo（元枢） threads, or delegate complex research tasks to Allo（元枢）. Also use when the user mentions deerflow, deer flow, allo, 元枢, or wants to run a deep research task that Allo（元枢） can handle."
---

# Allo（元枢） Skill

Communicate with a running Allo（元枢） instance via its HTTP API. Allo（元枢） is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Allo（元枢） exposes two API surfaces behind an Nginx reverse proxy:
...
- If health check fails, Allo（元枢） is not running. Inform the user they need to start it.
