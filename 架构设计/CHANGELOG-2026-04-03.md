...
| Skill Catalog 决议 | 90% | 🆕 从 70% 提升 |
| Soul per-user | 70% | — |
| MCP per-user | 60% | — |
| Skills custom per-user | 65% | — |
| Marketplace seed 幂等 | 0% | 数据已清理，逻辑待修 |
| Nginx auth_request | 60% | — |

## 下一步优先级

1. **P0**: Marketplace seed 幂等化
2. **P0**: Nginx auth_request 配置（让 LangGraph 流式链路也鉴权）
3. **P1**: MCP per-user 接通（`langgraph_runtime.py` 预解析 user MCP config）
4. **P1**: Skills custom 目录确认接通
5. **P2**: Sandbox Docker per-user pool
6. **P2**: 生产部署（systemd + HTTPS + 域名）
